...
BACKEND_DIR = TOOLS_DIR.parents[1]
...
def load_summaries(user_request: str) -> tuple[list[dict], list[str]]:
    if not SUMMARY_INDEX.is_file():
        raise FileNotFoundError(
            ...
            "Hãy chạy python -m backend.app.data_processing.summarize trước."
        )
    ...
    for day, entries in index["days"].items():
        ...
        for entry in entries:
            ...
            if not path.is_file():
                raise FileNotFoundError(
                    f"Thiếu {path}. Hãy chạy pipeline summarize trước."
                )
            ...
            if metadata.get("schema_version") != 2:
                raise ValueError(
                    ...
                    "Hãy chạy pipeline summarize để tạo lại."
                )
...
def generate_quiz(user_request: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "pip install -r codebase/backend/requirements.txt"
        ) from exc
